...
public:
    static void ConnectionFunction()
    {
        ...
        conn = mysql_real_connect(conn, "localhost", "root", "", "travelmanagementdb", 0, NULL, 0);
        if (conn)
        {
...
void NewTravelTrip()
{
    ...
    qstate = mysql_query(conn, "select * from traveltrip");
    if (!qstate)
    {
    ...
    string insert_query = "insert into travelusers (t_username, t_useraddress, t_userphone, t_useremail, t_usertripdate, t_usertripplan, t_usertripcost) values ('"+userName+"','"+userAddress+"','"+userPhone+"','"+userEmail+"','"+userTripDate+"','"+userTripPlan+"','"+userTripCost+"')";
...
void ShowUsers()
{
    ...
    qstate = mysql_query(conn, "select * from travelusers");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        while ((row = mysql_fetch_row(res)))
...
void FindUser()
{
    ...
    cout << "Enter User Name: ";
    getline(cin, 1input);
    string findbyname_query = "select * from travelusers where t_username like '%"+input+"%'";
...
void EditUser()
{
    ...
    if (HaveException == false)
    {
        ...
        {
            string findbyid_query = "select * from travelusers where t_id = '"+strid+"'";
            ...
            string update_query = "update travelusers set t_username = '"+userName+"', t_useraddress = '"+userAddress+"', t_userphone = '"+userPhone+"', t_useremail = '"+userEmail+"', t_usertripdate = '"+userTripDate+"', t_usertripplan = '"+userTripPlan+"', t_usertripcost = '"+userTripCost+"' where t_id = '"+strid+"'";
...
void DeleteUser()
{
    ...
    if (HaveException == false)
    {
        ...
        if (NotInDatabase == false)
        {
            string delete_query = "delete from travelusers where t_id = '"+strid+"'";
...
void AddTrip()
{
    ...
    string insert_query = "insert into traveltrip (t_trip) values ('"+tripName+"')";
...
void EditTrip()
{
    ...
    if (HaveException == false)
    {
        ...
        if (NotInDatabase == false)
        {
            string findbyid_query = "select * from traveltrip where t_id = '"+strid+"'";
            const char* qi = findbyid_query.c_str();
            qstate = mysql_query(conn, qi);
            ...
            string update_query = "update traveltrip set t_trip = '"+tripName+"' where t_id = '"+strid+"'";
            const char* qu = update_query.c_str();
            qstate = mysql_query(conn, qu);
...
void DeleteTrip()
{
    ...
    if (HaveException == false)
    {
      ...
      11      }else
        ...
        if (NotInDatabase == false)
        {
            string delete_query = "delete from traveltrip where t_id = '"+strid+"'";
            const char* qd = delete_query.c_str();
            qstate = mysql_query(conn, qd);
